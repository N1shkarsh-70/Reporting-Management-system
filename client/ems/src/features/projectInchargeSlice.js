...
export const getallProjectIncharge = createAsyncThunk("admin/getAllProjectIncharge", async (_, {rejectWithValue, getState}) => {
  ...
  const response = await axios.get(" https://reporting-management-system.onrender.com/api/superadmin/get-allprojectIncharge",{
  ...
  const response = await axios.post(" https://reporting-management-system.onrender.com/api/superadmin/add-projectIncharge", payload,{
  ...
  export const changeRole = createAsyncThunk("admin/changeRole", async (payload, {rejectWithValue, getState}) => {
    try{
        const token= getState().auth.token
  const response = await axios.post(" https://reporting-management-system.onrender.com/api/superadmin/changeRole", payload,{
    headers: {Authorization: `Bearer ${token}`},
  });
  console.log(response);
