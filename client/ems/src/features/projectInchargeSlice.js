...
export const getallProjectIncharge = createAsyncThunk("admin/getAllProjectIncharge", async (_, {rejectWithValue, getState}) => {
  ...
  const response = await axios.get(" http://localhost:3000/api/superadmin/get-allprojectIncharge",{
  ...
  const response = await axios.post(" http://localhost:3000/api/superadmin/add-projectIncharge", payload,{
  ...
  export const changeRole = createAsyncThunk("admin/changeRole", async (payload, {rejectWithValue, getState}) => {
    try{
        const token= getState().auth.token
  const response = await axios.post(" http://localhost:3000/api/superadmin/changeRole", payload,{
    headers: {Authorization: `Bearer ${token}`},
  });
  console.log(response);
