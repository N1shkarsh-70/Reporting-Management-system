...
export const fetchSiteEngineers = createAsyncThunk(
  ...
  async (_, { getState, rejectWithValue }) => {
    try {
      ...
      const response = await axios.get(" http://localhost:3000/api/superadmin/get-allsiteengineers", {
...
export const fetchActiveSiteEngineers = createAsyncThunk(
  ...
  async (_, { getState, rejectWithValue }) => {
    try {
      const token = getState().auth.token; // Get token from Redux store
      const response = await axios.get(" http://localhost:3000/api/superadmin/get-activeEngineers", {
        headers: { Authorization: `Bearer ${token}` }, // Attach token in headers
      });
...
export const getEngineerNamesByIds = createAsyncThunk("admin/getPlazaNames", async (engineerIds, { rejectWithValue, getState }) => {
  try {
    ...
    const response = await axios.post(" http://localhost:3000/api/superadmin/get-nameById", {engineerIds: formattedEngineerIds}  , {
    ...
    async (payload, { rejectWithValue, getState }) => {
      try {
        ...
        const response = await axios.post(
          " http://localhost:3000/api/superadmin/add-engineer",
          payload,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
...
export const deleteSiteEngineer = createAsyncThunk(
  ...
  async (username, { getState, rejectWithValue }) => {
    try {
      ...
      await axios.delete(` http://localhost:3000/api/superadmin/delete-engineer/${username}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  async ({ id, updatedData }, { getState, rejectWithValue }) => {
    try {
      ...
      const response = await axios.put(
        ` http://localhost:3000/api/superadmin/update-siteengineer/${id}`,
        updatedData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
