...
export const fetchPlazas = createAsyncThunk("plaza/fetchPlazas", async (_, { getState, rejectWithValue }) => {
    try {
      ...
      const response = await axios.get(" https://reporting-management-system.onrender.com/api/superadmin/get-allplaza", {
  ...
  export const fetchPlazasByProjectId = createAsyncThunk("plaza/fetchPlazasByProjectId", async (projectId, { getState, rejectWithValue }) => {
    try {
      ...
      const token = getState().auth.token; // Get token from auth state
      const response = await axios.get(` https://reporting-management-system.onrender.com/api/superadmin/get-plazasByProject/${projectId}`, {
        headers: { Authorization: `Bearer ${token}` }, // Attach token in headers
      });
...
export const getPlazaNamesByIds = createAsyncThunk("admin/getPlazaNames", async (plazaIds, { rejectWithValue, getState }) => {
  try {
    ...
    const response = await axios.post(" https://reporting-management-system.onrender.com/api/superadmin/get-plazaNames", {plazaIds: formattedPlazaIds}  , {
  ...
  export const addPlaza= createAsyncThunk("plaza/addPlaza", async (payload, {rejectWithValue,getState})=>{
    try{
    const token = getState().auth.token; // Get token from auth state
    const response= await axios.post(" https://reporting-management-system.onrender.com/api/superadmin/add-plaza", payload,{
      headers: { Authorization: `Bearer ${token}` }, // Attach token in headers
    })
...
export const deletePlaza = createAsyncThunk("plaza/deletePlaza", async (id) => {
  await axios.delete(` https://reporting-management-system.onrender.com/superadmin/delete-plaza/${id}`);
  return id;
});

export const updatePlaza = createAsyncThunk("plaza/updatePlaza", async ({ id, updatedData }) => {
  const response = await axios.put(` https://reporting-management-system.onrender.com/api/superadmin/update-plaza/${id}`, updatedData);
  return response.data;
});
